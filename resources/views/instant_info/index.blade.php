    ...
    */  
	setTimeout(function(){
		// $.get( "{!! './js/air_quality_station_of_geographic_information.json' !!}", function(data) {
			try {
    			$.each(stationgeo, function(i, name){
    				markers[i] = new google.maps.Marker({
    					position:new google.maps.LatLng(stationgeo[i].TWD97Lat, stationgeo[i].TWD97Lon),
    					map:map,
    					title:stationgeo[i].SiteName,
    					icon:icon[i]
    				});
    			...
    			*/
    			google.maps.event.addListener(markers[i], 'click', function(i) {
    				    return function() {
                            var chart = new setChart(stationgeo[i].SiteName);                            
